...
public final class AMethodParamUnderTest {
    ...
    @SuppressWarnings({ "unchecked" })
    private MethodParam createMethodParam(final Class<?> factoryClass, final String factoryMethod, final Class<?> paramType,
                                          final Object arg) {
        return new MethodParamBuilder().
                forInterface(factoryClass).
                havingMethod(factoryMethod, paramType).
                withArgument(arg).
                build();
    }
}
